...
def split_continuous(data_matrix, feature_index, value):
    ...
    left_indexes = np.where(value_list <= value)[0]
    left_matrix = data_matrix[left_indexes]

    right_indexes = np.where(value_list > value)[0]
...
def split_discrete(data_matrix, feature_index, value):
    equal_indexes = np.where(data_matrix[:, feature_index] == value)[0]
    unequal_indexes = np.where(data_matrix[:, feature_index] != value)[0]
    equal_matrix = data_matrix[equal_indexes, :]
    eq_split_matrix = np.hstack((equal_matrix[:, :feature_index], equal_matrix[:, feature_index+1:]))
    unequal_matrix = data_matrix[unequal_indexes, :]
